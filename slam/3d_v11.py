...
class LookaheadSLAM:
    ...
    Uses CLAHE preprocessing + ORB + GFTT + FAST for robust feature detection
    """
    ...
    def __init__(
        ...
        rotation_threshold=0.0015,
        min_depth_change=0.02,
    ...
    def update(self, gray, depth_map):
        ...
        if self.prev_gray is not None and self.lookahead_depth_prev is not None:
            ...
            if status is not None and status[0][0] == 1:
                ...
                # Translation estimation (FORWARD-ONLY)
                forward_distance = 0.0
                depth_diff = self.lookahead_depth_prev - self.lookahead_depth_curr

                if 0.05 > abs(depth_diff) > self.min_depth_change:
    ...
    def draw_top_view(self, scale=100, size=900, fps=None):
        ...
        cv2.putText(canvas, "ROVER STREAM | Enhanced: CLAHE+ORB+GFTT+FAST",
    ...
    def run_slam(self, source=0, target_w=640, target_h=480, fx=None, fy=None, use_rover_stream=False, rover_url=None):
        ...
        print("=" * 70)
        print("✅ Enhanced feature detection: CLAHE + ORB + GFTT + FAST")
        ...
        while True:
            ...
            cv2.imshow("Top-View Map (Enhanced Detection)", top_map)
        ...
        print(f"   Feature detection: Enhanced (CLAHE + ORB + GFTT + FAST)")
